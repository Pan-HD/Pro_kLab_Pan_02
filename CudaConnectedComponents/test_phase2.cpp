...
int main()
{
    try
    {
        ...
        std::vector<Npp32u> hLabel(width * height);

        CUDA_CHECK(cudaMemcpy(
            hLabel.data(),
            dLabel,
            width * height * sizeof(Npp32u),
            cudaMemcpyDeviceToHost
        ));

        std::cout << "\nMax Label = " << maxLabel << std::endl;

        std::cout << "\nCompressed Label Image:" << std::endl;
        for (int y = 0; y < height; ++y)
        {
            for (int x = 0; x < width; ++x)
            {
                unsigned int v = hLabel[y * width + x];
                if (v == 0)
                    std::cout << ". ";
                else
                    std::cout << v << " ";
