...
#include "./CudaConnectedComponents/cuda_cc_output.h"
...
static void printBinaryMat(const cv::Mat& mat)
{
    for (int y = 0; y < mat.rows; ++y)
    {
        for (int x = 0; x < mat.cols; ++x)
            std::cout << (mat.at<uchar>(y, x) > 0 ? "1 " : ". ");
        std::cout << std::endl;
    }
}
...
int main()
{
    try
    {
        ...
        unsigned char* dFiltered = nullptr;

        generateFilteredBinaryCUDA(
            dLabel,
            dRemoveMask,
            width,
            height,
        ...
        cv::Mat hFiltered(height, width, CV_8UC1);
        ...
        std::cout << "\nFiltered Binary:" << std::endl;
        printBinaryMat(hFiltered);

        freeFilteredBinaryCUDA(dFiltered);
