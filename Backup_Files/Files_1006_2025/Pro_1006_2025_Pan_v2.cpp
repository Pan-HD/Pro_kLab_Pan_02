// Pro_1006_2025_Pan_v1_fixed.cpp
// Fixed version: GP + GA hybrid with performance and correctness fixes.
//
// Key fixes:
// - unified RNG (rand_int / rand_real) for reproducibility
// - DIFF_PROCESS caches child subtree results (avoid repeated executeTree per pixel)
// - ensure node arity (children count) after crossover/mutation
// - GA optimizes current elite and protects it for next generation
// - enforce binary output before metric evaluation
// - various RNG replacements and small robustness fixes
    ...
    for (int idxProTimes = 0; idxProTimes < sysRunTimes; idxProTimes++) {
        ...
        if (indFValInfo[curMaxFvalIdx][numSets] > curThreshFVal) {
            ...
            for (int idxSet = 0; idxSet < numSets; idxSet++) {
                ...
                sprintf_s(imgName_pro[idxSet], "./imgs_1006_2025_v1/output/img_0%d/Gen-Final-t%d.png", idxSet + 1, idxProTimes + 1);
