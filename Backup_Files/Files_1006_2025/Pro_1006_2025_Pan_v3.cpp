...
#define INITIAL_BIAS_THRESHOLD 0.15
#define BIAS_DECAY 0.95
...
void mutate(shared_ptr<TreeNode>& root, int maxDepth = MAX_DEPTH) {
    ...
    case 1: { // insert above target
        ...
        if (g_paramDesc.count(newNode->type)) {
            //auto pd = g_paramDesc[newNode->type];
            //newNode->params.resize(pd.n);
...
void multiProcess(Mat imgArr[][2]) {
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_1006_2025_v3/output/f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_1006_2025_v3/output/maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree = nullptr;
    errno_t err3 = fopen_s(&fl_printTree, "./imgs_1006_2025_v3/output/printed_tree.txt", "w");
    ...
    for (int idxProTimes = 0; idxProTimes < sysRunTimes; idxProTimes++) {
            ...
            if (numGen != GENERATIONS - 1) {
                if (ENABLE_GA && bias < biasThreshold) {
                    ...
                    if (!bestParams.empty()) {
                        ...
                        cout << "[PT-ACTIT] GA wrote optimized params to elite index " << eliteIndex << " and protected until gen " << numGen + 1 << endl;
                    }
        ...
        if (indFValInfo[curMaxFvalIdx][numSets] > curThreshFVal) {
            ...
            for (int idxSet = 0; idxSet < numSets; idxSet++) {
                ...
                sprintf_s(imgName_pro[idxSet], "./imgs_1006_2025_v3/output/img_0%d/Gen-Final-t%d.png", idxSet + 1, idxProTimes + 1);
                imwrite(imgName_pro[idxSet], res);
                Mat concat;
                vector<Mat> vec = { res, imgArr[idxSet][1] };
                hconcat(vec, concat);
                sprintf_s(imgName_final[idxSet], "./imgs_1006_2025_v3/output/img_0%d/imgs_final-t%d.png", idxSet + 1, idxProTimes + 1);
...
int main(void) {
    ...
    if (numSets == 1) {
        sprintf_s(inputPathName_ori, "./imgs_1006_2025_v3/input/oriImg_0%d.png", idSet);
        sprintf_s(inputPathName_tar, "./imgs_1006_2025_v3/input/tarImg_0%d.png", idSet);
        imgArr[0][0] = imread(inputPathName_ori, 0);
        imgArr[0][1] = imread(inputPathName_tar, 0);
    }
    else {
        for (int i = 0; i < numSets; i++) {
            sprintf_s(inputPathName_ori, "./imgs_1006_2025_v3/input/oriImg_0%d.png", i + 1);
            sprintf_s(inputPathName_tar, "./imgs_1006_2025_v3/input/tarImg_0%d.png", i + 1);
