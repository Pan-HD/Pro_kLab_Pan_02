...
bool isSafeValType(FilterType t) {
    // GP Only: current version uses safe values for all generated parameters.
    return true;
}
...
Mat executeTree(const shared_ptr<TreeNode>& node, const Mat& input) {
    ...
    case CC_FILTER: {
        // CPU path is kept only for legacy fallback/debug.
        // The current test path uses executeCCFilterCUDA().
        Mat child = executeTree(node->children[0], input);
        return child;
    }
    case BITWISE_AND: {
