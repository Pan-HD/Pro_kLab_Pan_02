...
// A: ENABLE_CC_FILTER = 1， B: ENABLE_CC_FILTER = 0
...
/**
 * @brief 重复计算同一个个体的适应度，检查 CUDA 计算结果是否具有确定性。
 *
 * 对同一棵 GP 树和相同训练数据多次调用 CAL_SCORE()，记录每次得到的
 * 适应度值，并统计最小值、最大值及其差值。若多次计算结果存在超出容差
 * 的差异，则说明适应度计算可能存在非确定性（如 CUDA 异步执行、浮点
 * 归约顺序、未初始化显存等问题）。
 *
 * @param tree        待测试的 GP 个体（树结构）
 * @param imgArr      训练图像数据
 * @param repeatCount 重复计算次数（默认 100 次）
 */
...
void multiProcess(Mat imgArr[][2]) {
    ...
    */
    ...
    // repeat score for 100 times
    vector<string> eliteTreeFiles = {
        "./imgs_0630_2026_v1/input/elite_trees/best_01_2.88.txt"
    };
    auto testTree = loadTreeFromFile(eliteTreeFiles[0]);
