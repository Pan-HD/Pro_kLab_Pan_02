    ...
    int numInd)
{
    ...
    return calScoreByIndCUDA_fused(node, imgArr, numInd);
#else
    return calScoreByIndCUDA_old(node, imgArr, numInd);
#endif
}
#endif
...
{
    double oldScore =
        calScoreByIndCUDA_old(tree, imgArr, -1);

    double fusedScore =
        calScoreByIndCUDA_fused(tree, imgArr, -1);

    double diff = fabs(oldScore - fusedScore);
    ...
    if (diff > 1e-9) {
        printf("[ERROR] old/fused score mismatch detected.\n");
    }
}

#if USE_CUDA
void debugCompareOldAndFusedSameOutput(
    const shared_ptr<TreeNode>& tree,
    Mat imgArr[][2])
{
    double oldSum = 0.0;
    ...
    for (int idxSet = 0; idxSet < TRAIN_SIZE; ++idxSet) {
        ...
        cv::cuda::GpuMat resImg =
            executeTreeCUDA(tree, gImgArr[idxSet][0], ctx);

        ctx.hold(resImg);
        getCudaStream().waitForCompletion();

        bool oldBinary = isBinaryImageGPU(resImg);

        MetricsGPU oldM{};
        MetricsGPUFused fusedM{};

        if (!oldBinary) {
            oldSum = 0.01;
            fusedM = calcMetricsOneGPUFused(
                resImg,
                gTarImgArr[idxSet],
                FG_PIXEL,
                getCudaStream());
            ...
            printf(
                "[SameOutput] idx=%d non-binary old=0.01 fused_invalid=%lld\n",
                idxSet,
                fusedM.invalid);
        ...
        long long oldTp = oldM.tp;
        long long oldFp = oldM.fp;
        long long oldFn = oldM.fn;

        long long fusedTp = fusedM.tp;
        long long fusedFp = fusedM.fp;
        long long fusedFn = fusedM.fn;

        if (oldTp == 0) oldTp++;
        if (oldFp == 0) oldFp++;
        if (oldFn == 0) oldFn++;

        if (fusedTp == 0) fusedTp++;
        if (fusedFp == 0) fusedFp++;
        if (fusedFn == 0) fusedFn++;

        double oldPrecision =
            double(oldTp) / double(oldTp + oldFp);

        double oldRecall =
            double(oldTp) / double(oldTp + oldFn);

        double oldF1 =
            calculateF1Score(oldPrecision, oldRecall);

        double fusedPrecision =
            double(fusedTp) / double(fusedTp + fusedFp);

        double fusedRecall =
            double(fusedTp) / double(fusedTp + fusedFn);

        double fusedF1 =
            calculateF1Score(fusedPrecision, fusedRecall);

        oldSum += oldF1;
        fusedSum += fusedF1;

        printf(
            "[SameOutput-Image] idx=%d "
            "old(tp=%d fp=%d fn=%d f1=%.12f) "
            "fused(tp=%lld fp=%lld fn=%lld invalid=%lld f1=%.12f) "
            "diff=%.12f\n",
            idxSet,
            oldM.tp,
            oldM.fp,
    ...
    printf(
        "[SameOutput-Total] old=%.12f fused=%.12f diff=%.12f\n",
        oldSum,
        fusedSum,
        fabs(oldSum - fusedSum)
    );

    if (fabs(oldSum - fusedSum) > 1e-9) {
        printf("[ERROR] Same-output old/fused metrics mismatch.\n");
    }
}
#endif

#if USE_CUDA
void debugRepeatScore100(
    const shared_ptr<TreeNode>& tree,
    Mat imgArr[][2],
    const char* tag = "RepeatScore100")
{
    double first = CAL_SCORE(tree, imgArr, -1);

    double minScore = first;
    double maxScore = first;
    ...
    for (int r = 1; r < 100; ++r) {
        ...
        printf(
            "[%s] run=%03d score=%.12f diffFromFirst=%.12f\n",
            tag,
            r,
            s,
            fabs(s - first)
        ...
        if (fabs(s - first) > 1e-12) {
            printf(
                "[ERROR] Repeat score mismatch: run=%d first=%.12f current=%.12f diff=%.12f\n",
                r,
                first,
                s,
                fabs(s - first)
    ...
    printf(
        "[%s-Summary] first=%.12f min=%.12f max=%.12f range=%.12f\n\n",
        tag,
        first,
        minScore,
        maxScore,
...
#if USE_CUDA
void debugCompareTwoExecutions(
    const shared_ptr<TreeNode>& tree,
    Mat imgArr[][2],
    int treeId = -1)
{
    bool treeHasError = false;

    for (int idxSet = 0; idxSet < TRAIN_SIZE; ++idxSet) {
        CudaEvalContext ctx1;
        CudaEvalContext ctx2;

        cv::cuda::GpuMat res1 =
            executeTreeCUDA(tree, gImgArr[idxSet][0], ctx1);
        ctx1.hold(res1);
        getCudaStream().waitForCompletion();

        cv::cuda::GpuMat res2 =
            executeTreeCUDA(tree, gImgArr[idxSet][0], ctx2);
        ctx2.hold(res2);
        getCudaStream().waitForCompletion();

        cv::cuda::GpuMat diffMask;
        cv::cuda::compare(
            res1,
            res2,
            diffMask,
        ...
        int diffPixels =
            cv::cuda::countNonZero(diffMask);

        printf(
            "[TwoExec] tree=%d idx=%d diffPixels=%d total=%d\n",
            treeId,
            idxSet,
            diffPixels,
            res1.rows * res1.cols
        ...
        if (diffPixels > 0) {
            ...
            printf(
                "[ERROR] executeTreeCUDA is not deterministic. "
                "tree=%d idx=%d diffPixels=%d\n",
                treeId,
                idxSet,
                diffPixels
            );
    ...
    if (treeHasError) {
        printf(
            "[ERROR-TREE-STATS] tree=%d "
            "CC=%d BILATERAL=%d CANNY=%d SOBEL_X=%d SOBEL_Y=%d MED=%d\n",
            treeId,
            countNodeType(tree, CC_FILTER),
            countNodeType(tree, BILATERAL_FILTER),
            countNodeType(tree, CANNY),
            countNodeType(tree, SOBEL_X),
            countNodeType(tree, SOBEL_Y),
            countNodeType(tree, MED_BLUR)
        );

        printf("\n[ERROR-TREE] tree=%d\n", treeId);
        printTree(tree, 0, stdout, stdout);
        printf("[ERROR-TREE-END]\n\n");
    }
}
#endif

#if USE_CUDA
void debugCompareCCFilterOnly(
    const shared_ptr<TreeNode>& ccTree,
    Mat imgArr[][2],
    int treeId = -1)
{
    CV_Assert(ccTree);
    CV_Assert(ccTree->type == CC_FILTER);
    CV_Assert(!ccTree->children.empty());

    for (int idxSet = 0; idxSet < TRAIN_SIZE; ++idxSet) {
        CudaEvalContext ctx;

        cv::cuda::GpuMat child =
            executeTreeCUDA(ccTree->children[0], gImgArr[idxSet][0], ctx);

        ctx.hold(child);
        getCudaStream().waitForCompletion();

        cv::cuda::GpuMat out1 =
            executeCCFilterCUDA(child, ccTree->params);
        cudaDeviceSynchronize();
        ...
        int diffPixels = cv::cuda::countNonZero(diffMask);

        printf(
            "[CCOnly-TwoExec] tree=%d idx=%d diffPixels=%d total=%d\n",
            treeId,
            idxSet,
            diffPixels,
            out1.rows * out1.cols
...
shared_ptr<TreeNode> makeCCFilterCannyTree()
{
    auto root = make_shared<TreeNode>();
    root->type = CC_FILTER;
    root->params = g_paramDesc_safeVal[CC_FILTER];

    auto canny = make_shared<TreeNode>();
    canny->type = CANNY;
    canny->params = g_paramDesc_safeVal[CANNY];

    auto input = make_shared<TreeNode>();
    input->type = TERMINAL_INPUT;

    canny->children.push_back(input);
    root->children.push_back(canny);
...
void multiProcess(Mat imgArr[][2]) {
    ...
    //#if USE_CUDA
    //    for (int i = 0; i < 100; ++i) {
    //        auto t = generateRandomTree();
    //        confirmDepth(t);
    //        debugCompareTwoExecutions(t, imgArr, i);
    //    }
    //    return;
    //#endif
...
#if USE_CUDA && RUN_TEST_OLD_FUSED_SAME_OUTPUT
    {
        ...
        while (tested < 100 && attempts < 2000) {
            attempts++;

            auto t = generateRandomTree();
            confirmDepth(t);

            if (countNodeType(t, CC_FILTER) == 0) {
                continue;
            }

            printf(
                "\n[OldFused-SameOutput-Start] tree=%d attempt=%d CC=%d depth=%d\n",
                tested,
                attempts,
                countNodeType(t, CC_FILTER),
                getTreeMaxDepth(t)
            );

            debugCompareOldAndFusedSameOutput(t, imgArr);

            tested++;
        }

        printf(
            "\n[OldFused-SameOutput-Summary] testedCC=%d attempts=%d\n",
            tested,
            attempts
        );
...
#if USE_CUDA && RUN_TEST_REPEAT_SCORE_100
    {
        ...
        for (int attempt = 0; attempt < 3000; ++attempt) {
            auto cand = generateRandomTree();
            confirmDepth(cand);

            if (countNodeType(cand, CC_FILTER) == 0) {
                continue;
            }

            double s = CAL_SCORE(cand, imgArr, -1);

            printf(
                "[RepeatScore100-Select] attempt=%d CC=%d depth=%d score=%.12f\n",
                attempt,
                countNodeType(cand, CC_FILTER),
                getTreeMaxDepth(cand),
                s
            );
            ...
            if (s > 0.010000000001) {
                ...
                printf(
                    "[RepeatScore100] selected VALID attempt=%d CC=%d depth=%d score=%.12f\n",
                    attempt,
                    countNodeType(t, CC_FILTER),
                    getTreeMaxDepth(t),
                    selectedScore
        ...
        printf("\n[RepeatScore100-ValidTree]\n");
        printTree(t, 0, stdout, stdout);

        debugRepeatScore100(t, imgArr, "RepeatScore100-ValidRandomCC");
...
#if USE_CUDA && RUN_TEST_RANDOM_TREE_TWOEXEC
    {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            int ccCount = countNodeType(t, CC_FILTER);
            if (ccCount > 0) {
                withCC++;
            }

            printf(
                "\n[RandomTree-TwoExec-Start] tree=%d CC=%d depth=%d\n",
                i,
                ccCount,
                getTreeMaxDepth(t)
            );
        ...
        printf(
            "\n[RandomTree-TwoExec-Summary] tested=%d withCC=%d\n",
            tested,
            withCC
        );
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_0630_2026_v1/output/train_output/f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0630_2026_v1/output/train_output/maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_sys = nullptr;
    errno_t err3 = fopen_s(&fl_printTree_sys, "./imgs_0630_2026_v1/output/train_output/printed_tree_sys.txt", "w");
    if (err3 != 0 || fl_printTree_sys == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_read = nullptr;
    errno_t err4 = fopen_s(&fl_printTree_read, "./imgs_0630_2026_v1/output/train_output/printed_tree_read.txt", "w");
    if (err4 != 0 || fl_printTree_read == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logOptiGA = nullptr;
    errno_t err5 = fopen_s(&fl_logOptiGA, "./imgs_0630_2026_v1/output/train_output/log_opti_ga.txt", "a");
    ...
    vector<string> eliteTreeFiles = {
        "./imgs_0630_2026_v1/input/elite_trees/best_01_7.17.txt",
        "./imgs_0630_2026_v1/input/elite_trees/best_02_6.95.txt",
        "./imgs_0630_2026_v1/input/elite_trees/best_03_6.82.txt"
    };
            ...
            if (genInfo[numGen].eliteFValue >= GA_TRIGGER_THRESH && bias < biasThreshold) {
                ...
                if (lastGenFailGA < 0 || (lastGenFailGA + NUM_DELAY_GA) < numGen) {
                    ...
                    if (!bestParams.empty()) {
                        ...
                        if (CAL_SCORE(eliteTreeClone, imgArr, -1) <= genInfo[numGen].eliteFValue) {
                            ...
                            fitness_opt_GA = CAL_SCORE(population[eliteIndex], imgArr, -1);
...
int main(void) {
    ...
    /*
    if (TRAIN_SIZE == 1) {
        sprintf_s(inputPathName_ori, "./imgs_0630_2026_v1/input/oriImg_0%d.png", idSet);
        sprintf_s(inputPathName_tar, "./imgs_0630_2026_v1/input/tarImg_0%d.png", idSet);
        imgArr[0][0] = imread(inputPathName_ori, 0);
        imgArr[0][1] = imread(inputPathName_tar, 0);
    }
    else {
        for (int i = 0; i < TRAIN_SIZE; i++) {
            sprintf_s(inputPathName_ori, "./imgs_0630_2026_v1/input/oriImg_0%d.png", i + 1);
            sprintf_s(inputPathName_tar, "./imgs_0630_2026_v1/input/tarImg_0%d.png", i + 1);
    ...
    */
    ...
    for (int i = 0; i < TRAIN_SIZE; i++) {
        sprintf_s(inputPathName_ori, "./imgs_0630_2026_v1/input/train/positive/images/crack_%05d.png", IMG_START_IDX + i);
        sprintf_s(inputPathName_tar, "./imgs_0630_2026_v1/input/train/positive/masks/crack_%05d.png", IMG_START_IDX + i);
        imgArr[i][0] = imread(inputPathName_ori, 0);
        imgArr[i][1] = imread(inputPathName_tar, 0);
    }
