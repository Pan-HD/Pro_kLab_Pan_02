...
int lastGenFailGA = -1;
// bool isPruneSuccess = false;
// double scoreAfterSucPrune = 0.01;
int countPruneSum = 0;
...
cv::cuda::GpuMat executeTreeCUDA(const shared_ptr<TreeNode>& node, const cv::cuda::GpuMat& input) {
    ...
    case GAUSSIAN_BLUR: {
        ...
        double sigma = node->params.size() > 1 ? node->params[1] : 1.5;
        cv::cuda::GpuMat dst;
        auto filter = cv::cuda::createGaussianFilter(child.type(), child.type(), Size(k, k), sigma);
        filter->apply(child, dst);
        return dst;
    }
    case SOBEL_X: {
        auto child = executeTreeCUDA(node->children[0], input);
        int k = max(1, int(node->params.size() > 0 ? int(node->params[0]) : 3));
        if ((k % 2) == 0) k |= 1;
        cv::cuda::GpuMat dst16;
        cv::cuda::GpuMat dst8;
        auto sobel = cv::cuda::createSobelFilter(CV_8UC1, CV_16S, 1, 0, k);
        sobel->apply(child, dst16);
        cv::cuda::abs(dst16, dst16);
        dst16.convertTo(dst8, CV_8U);
        return dst8;
    }
    case SOBEL_Y: {
        auto child = executeTreeCUDA(node->children[0], input);
        ...
        cv::cuda::GpuMat dst16;
        cv::cuda::GpuMat dst8;
        auto sobel = cv::cuda::createSobelFilter(CV_8UC1, CV_16S, 0, 1, k);
        sobel->apply(child, dst16);
        cv::cuda::abs(dst16, dst16);
        dst16.convertTo(dst8, CV_8U);
        return dst8;
    ...
    case CANNY: {
        ...
        double t2 = node->params.size() > 1 ? node->params[1] : 200;
        cv::cuda::GpuMat dst;
        auto canny = cv::cuda::createCannyEdgeDetector(t1, t2);
        canny->detect(child, dst);
        return dst;
    }
    case THRESHOLD: {
        auto child = executeTreeCUDA(node->children[0], input);
        double th = node->params.size() > 0 ? node->params[0] : 127;
        cv::cuda::GpuMat dst;
        cv::cuda::threshold(child, dst, th, 255, THRESH_BINARY);
        return dst;
    }
    case BITWISE_AND: {
    ...
    case BITWISE_XOR: {
        ...
        auto b = executeTreeCUDA(node->children[1], input);
        cv::cuda::GpuMat dst;
        cv::cuda::bitwise_xor(a, b, dst);
        return dst;
    }
    case BITWISE_NOT: {
        auto a = executeTreeCUDA(node->children[0], input);
        cv::cuda::GpuMat dst;
        cv::cuda::bitwise_not(a, dst);
        return dst;
    }
                    /*
                        CONTOUR_PROCESS
...
double calScoreByIndCUDA(const shared_ptr<TreeNode>& node, Mat imgArr[][2], int numInd) {
    ...
    for (int i = 0; i < numSets; i++) {
        cv::cuda::GpuMat gsrc;
        gsrc.upload(imgArr[i][0]);
        cv::cuda::GpuMat gres = executeTreeCUDA(node, gsrc);
        gres.download(resImg[i]);
    }
    return calculateMetrics(resImg, tarImg, numInd);
}
...
void collectSubtreeNodes(
    ...
    vector<shared_ptr<TreeNode>>& out)
{
    if (!node) return;
...
PruneResult pruneIntrons(shared_ptr<TreeNode>& root, Mat imgArr[][2])
{
    ...
    if (scoreAfter >= scoreBefore)
    {
        root = rootPruned;
        //isPruneSuccess = true;
        //scoreAfterSucPrune = scoreAfter;
        return { true, scoreAfter };
    }
    // isPruneSuccess = false;
    return { false, 0.0 };
}
...
void multiProcess(Mat imgArr[][2]) {
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_0528_2026_v1/output/f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0528_2026_v1/output/maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_sys = nullptr;
    errno_t err3 = fopen_s(&fl_printTree_sys, "./imgs_0528_2026_v1/output/printed_tree_sys.txt", "w");
    if (err3 != 0 || fl_printTree_sys == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_read = nullptr;
    errno_t err4 = fopen_s(&fl_printTree_read, "./imgs_0528_2026_v1/output/printed_tree_read.txt", "w");
    if (err4 != 0 || fl_printTree_read == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logOptiGA = nullptr;
    errno_t err5 = fopen_s(&fl_logOptiGA, "./imgs_0528_2026_v1/output/log_opti_ga.txt", "a");
    if (err5 != 0 || fl_logOptiGA == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logPrune = nullptr;
    errno_t err6 = fopen_s(&fl_logPrune, "./imgs_0528_2026_v1/output/log_prune.txt", "a");
    ...
    /*
      Break-Point-01
    */
    // This variable is used to indicate whether the GA module was triggered in the previous generation and whether it improved the fitness.
    ...
    vector<string> eliteTreeFiles = {
        "./imgs_0528_2026_v1/input/elite_trees/best_01_6.82.txt",
        "./imgs_0528_2026_v1/input/elite_trees/best_02_6.39.txt",
        "./imgs_0528_2026_v1/input/elite_trees/best_03_6.25.txt"
    };
...
#pragma omp parallel for
    ...
    //if (indFValInfo[genInfo.back().eliteIndex][numSets] > curThreshFVal) {
    ...
    //        sprintf_s(imgName_pro[idxSet], "./imgs_0528_2026_v1/output/img_0%d/Gen-Final.png", idxSet + 1);
    //        imwrite(imgName_pro[idxSet], res);
    //        Mat concat;
    //        vector<Mat> vec = { res, imgArr[idxSet][1] };
    //        hconcat(vec, concat);
    //        sprintf_s(imgName_final[idxSet], "./imgs_0528_2026_v1/output/img_0%d/imgs_final.png", idxSet + 1);
    ...
    if (fl_maxFval) {
        // for (int i = 0; i <= numSets; i++) fprintf(fl_maxFval, "%.4f ", indFValInfo[curMaxFvalIdx][i]);
        for (int i = 0; i <= numSets; i++) fprintf(fl_maxFval, "%.4f ", indFValInfo[genInfo.back().eliteIndex][i]);
        fprintf(fl_maxFval, "\n");
    }
    // save final images for elite of last generation
    for (int idxSet = 0; idxSet < numSets; idxSet++) {
...
int main(void) {
    ...
    if (numSets == 1) {
        sprintf_s(inputPathName_ori, "./imgs_0528_2026_v1/input/oriImg_0%d.png", idSet);
        sprintf_s(inputPathName_tar, "./imgs_0528_2026_v1/input/tarImg_0%d.png", idSet);
        imgArr[0][0] = imread(inputPathName_ori, 0);
        imgArr[0][1] = imread(inputPathName_tar, 0);
    }
    else {
        for (int i = 0; i < numSets; i++) {
            sprintf_s(inputPathName_ori, "./imgs_0528_2026_v1/input/oriImg_0%d.png", i + 1);
            sprintf_s(inputPathName_tar, "./imgs_0528_2026_v1/input/tarImg_0%d.png", i + 1);
