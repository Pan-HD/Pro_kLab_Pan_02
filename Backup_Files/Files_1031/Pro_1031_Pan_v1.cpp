...
int main(void) {
    ...
    FILE* file = nullptr;
    // errno_t err = fopen_s(&file, "params.txt", "a"); 
    errno_t err = fopen_s(&file, "./imgs_Pro_GA/output/params.txt", "a");
    ...
    for (int numGen = 1; numGen <= num_gen; numGen++) {
        ...
        if (numGen % 10 == 0) {
            sprintf_s(imgName, "./imgs_Pro_GA/output/Gen-%d.png", numGen);
            imwrite(imgName, labelImg);
        }
    }
    ...
    hconcat(images, res);
    imgShow("res_p1", res);
    imwrite("./imgs_Pro_GA/imgs_final.png", res);
    fclose(file);
}