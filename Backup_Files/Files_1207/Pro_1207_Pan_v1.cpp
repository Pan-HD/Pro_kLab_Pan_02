...
// for storing the fitness value of 8(mod03: 7 -> 8) decision variables
gene h[num_ind][8];
...
Mat labeling(Mat img, int connectivity) {
    ...
    for (i = 1; i < label_num; i++)
    {
        ...
        if ((int)(label_cal / label_areaall) < linear)
        {
...
void singleProcess(Mat oriImg_g, Mat tarImg, Mat maskImg, int num_img) {
    ...
    char paramAddName[256]; // The address of single pair
    ...
    FILE* fp_d = nullptr;
    errno_t err1 = fopen_s(&fp_d, "./imgs_1107_v2/output/deci_vari.txt", "a");
    if (err1 != 0 || fp_d == nullptr) {
        perror("Cannot open the file");
        return;
    }
    ...
    fprintf(fp_d, "%d %d %d %d %d %d %d %d\n", fsize, binary, linear, abusolute_flag, erodedilate_sequence, filterswitch_flag, erodedilate_times, pixellabelingmethod);
    fclose(fp_d);
}

int main(void) {
    Mat imgArr[5][3]; // imgArr -> storing all images 5(5 pairs) * 3(ori, tar, mask)
