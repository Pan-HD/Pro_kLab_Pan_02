...
void pruneIntrons(shared_ptr<TreeNode>& root, Mat imgArr[][2])
{
    ...
    double scoreBefore = calScoreByInd(root, imgArr, -1);
    ...
    double scoreAfter = calScoreByInd(rootPruned, imgArr, -1);
...
void multiProcess(Mat imgArr[][2]) {
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_0519_2026_v1/output/f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0519_2026_v1/output/maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_sys = nullptr;
    errno_t err3 = fopen_s(&fl_printTree_sys, "./imgs_0519_2026_v1/output/printed_tree_sys.txt", "w");
    if (err3 != 0 || fl_printTree_sys == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_read = nullptr;
    errno_t err4 = fopen_s(&fl_printTree_read, "./imgs_0519_2026_v1/output/printed_tree_read.txt", "w");
    if (err4 != 0 || fl_printTree_read == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logOptiGA = nullptr;
    errno_t err5 = fopen_s(&fl_logOptiGA, "./imgs_0519_2026_v1/output/log_opti_ga.txt", "a");
    if (err5 != 0 || fl_logOptiGA == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logPrune = nullptr;
    errno_t err6 = fopen_s(&fl_logPrune, "./imgs_0519_2026_v1/output/log_prune.txt", "a");
    ...
    */
    ...
    vector<string> eliteTreeFiles = {
        "./imgs_0519_2026_v1/input/elite_trees/best_01_6.82.txt",
        "./imgs_0519_2026_v1/input/elite_trees/best_02_6.39.txt",
        "./imgs_0519_2026_v1/input/elite_trees/best_03_6.25.txt"
    };
    ...
    if (indFValInfo[curMaxFvalIdx][numSets] > curThreshFVal) {
        ...
        for (int idxSet = 0; idxSet < numSets; idxSet++) {
            ...
            sprintf_s(imgName_pro[idxSet], "./imgs_0519_2026_v1/output/img_0%d/Gen-Final.png", idxSet + 1);
            imwrite(imgName_pro[idxSet], res);
            Mat concat;
            vector<Mat> vec = { res, imgArr[idxSet][1] };
            hconcat(vec, concat);
            sprintf_s(imgName_final[idxSet], "./imgs_0519_2026_v1/output/img_0%d/imgs_final.png", idxSet + 1);
...
int main(void) {
    ...
    if (numSets == 1) {
        sprintf_s(inputPathName_ori, "./imgs_0519_2026_v1/input/oriImg_0%d.png", idSet);
        sprintf_s(inputPathName_tar, "./imgs_0519_2026_v1/input/tarImg_0%d.png", idSet);
        imgArr[0][0] = imread(inputPathName_ori, 0);
        imgArr[0][1] = imread(inputPathName_tar, 0);
    }
    else {
        for (int i = 0; i < numSets; i++) {
            sprintf_s(inputPathName_ori, "./imgs_0519_2026_v1/input/oriImg_0%d.png", i + 1);
            sprintf_s(inputPathName_tar, "./imgs_0519_2026_v1/input/tarImg_0%d.png", i + 1);
