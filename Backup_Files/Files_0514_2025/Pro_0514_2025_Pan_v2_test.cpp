...
#define numDV 18 // the nums of decision-variables
#define chLen 66 // the length of chromosome
...
// ["d", "sigmaColor", "sigmaSpace", "absoluteFlag", "threshVal", "ConProTimes"]
...
int info_len_dv[numDV] = { 4, 8, 7, 1, 8, 2, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4 };
...
void import_para(int idxInd) {
	...
	info_val_dv[0] = info_val_dv[0] * 2 + 1;
...
void imgSingleProcess(Mat& oriImg, Mat& resImg, int arr_val_dv[]) {
	...
	// GaussianBlur(oriImg, blurImg, Size(15, 15), 7.5);

	bilateralFilter(oriImg, blurImg, arr_val_dv[0], arr_val_dv[1], arr_val_dv[2]);

	// imgShow("res", blurImg);

	differenceProcess(blurImg, oriImg, diffImg, arr_val_dv[3]);

	// imgShow("res", diffImg);

	threshold(diffImg, biImg, arr_val_dv[4], 255, THRESH_BINARY);
