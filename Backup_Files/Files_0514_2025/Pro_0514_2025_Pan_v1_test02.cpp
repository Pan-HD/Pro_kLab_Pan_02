...
int info_val_dv[18] = { 1, 31, 0, 10, 1, 1, 3, 6, 9, 3, 3, 0, 1, 4, 10, 2, 7, 2 }; // Opt of GA for Rep on 0806
...
//int main(void) {
//	Mat oriImg = imread("./imgs_0514_2025_v1/input/oriImg_10.png", IMREAD_GRAYSCALE);
//	Mat resImg;
//	imgShow("ori", oriImg);
...
int main(void) {
	...
	for (int idxImg = 0; idxImg < 14; idxImg++) {
		if (idxImg < 9) {
			sprintf_s(outputPathName_res[idxImg], "./imgs_0806_2025_v0_test/output/Ver04/resImg_0%d.png", idxImg + 1);
		}
		else {
			sprintf_s(outputPathName_res[idxImg], "./imgs_0806_2025_v0_test/output/Ver04/resImg_%d.png", idxImg + 1);
		}
...
//	// cv::findContours(biImg, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
//	//cv::findContours(biImg, contours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
//
//	//cv::Mat filled = cv::Mat::ones(biImg.size(), CV_8UC1) * 255;
//
//	//int cnt = 0;
//	//for (const auto& contour : contours) {
//	//	//Rect bounding_box = boundingRect(contour);
...
//	//	//	drawContours(mask, vector<vector<Point>>{contour}, -1, Scalar(255), -1);
//	//	//}
//	//    // printf("area of eve-cons: %d\n", (int)contourArea(contour));
//	//	if (cnt == 19) break;
//	//	cv::drawContours(filled, vector<vector<Point>>{contour}, -1, cv::Scalar(0), cv::FILLED);
//	//	cnt++;
//	//}
//
//	//printf("nums of cons: %d\n", (int)contours.size());
//	//printf("nums of cons[0]: %d\n", (int)contourArea(contours[0]));
//
//
//
//	//cv::Mat filled = cv::Mat::ones(biImg.size(), CV_8UC1) * 255;  
//	//cv::drawContours(filled, contours, -1, cv::Scalar(0), cv::FILLED);  
//	// imgShow("res", filled);
//
//	resImg = biImg.clone();
//}
