...
void calculateMetrics(Mat metaImg[], Mat tarImg[], Mat maskImg[], int curCase) {
    //if (curCase == 40) {
    //    printf("ENTERING in case of 40\n");
    //}
    ...
    if (sum_f1 > bestCaseInfo.fValuArr[numSets]) {
...
void singleTimeProcess(int chromArr[], Mat imgArr[][3], int curCase) {
    phenotype(chromArr);
    import_para();
    ...
    calculateMetrics(labelImg, tarImg, maskImg, curCase);
}
...
void docAppend(FILE* file, int numPeriod, bestCaseType periodBestInfo) {
    fprintf(file, "-------best case in per-%d: -------\n", numPeriod);
    // if the best-case-arr is not be updated, then there is not a better res in this period
    fprintf(file, "BestCase: %d\n", periodBestInfo.curCase);
    // Writing the fvalue of cur-best-case in this period
    for (int i = 0; i <= numSets; i++) {
        fprintf(file, "%.4f ", periodBestInfo.fValuArr[i]);
    }
    fprintf(file, "\n");
    // Writing the info of 8 deci-varis ...
    for (int i = 0; i < 8; i++) {
        fprintf(file, "%d ", periodBestInfo.paramArr[i]);
    }
    fprintf(file, "\n");
}
...
int main(void) {
    ...
    int periodLen = 1048576;
    int numPeriod = 0;

    // declaring a temp object for function-docAppend()
    bestCaseType curBestCaseInfo;

    for (int caseIdx = 0; caseIdx < count; caseIdx++) {
        printf("curCase: %d\n", caseIdx);
        singleTimeProcess(chromArr, imgArr, caseIdx);
        if ((caseIdx + 1) % periodLen == 0) {

            // updating the info of the temp object
            curBestCaseInfo.curCase = bestCaseInfo.curCase;
            for (int i = 0; i <= numSets; i++) {
                curBestCaseInfo.fValuArr[i] = bestCaseInfo.fValuArr[i];
            }
            for (int i = 0; i < 8; i++) {
                curBestCaseInfo.paramArr[i] = bestCaseInfo.paramArr[i];
            }

            docAppend(file, numPeriod, curBestCaseInfo);
            numPeriod += 1;
        }
        arrIteration(chromArr, chLen);
    }

    fprintf(file, "-------Final Best Case-------\n");
    fprintf(file, "Case: %d\n", bestCaseInfo.curCase);
    for (int i = 0; i <= numSets; i++) {
