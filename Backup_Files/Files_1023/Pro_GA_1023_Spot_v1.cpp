...
#define mut 0.18 // the rate of mutation
...
void calculateMetrics(Mat metaImg, Mat tarImg, Mat mask, int numInd, int numGen) {
    ...
    for (int y = 0; y < metaImg.rows; y++) {
        for (int x = 0; x < metaImg.cols; x++) {
            ...
            bool isMetaImgWhite = (metaImg.at<uchar>(y, x) == 255);
            bool isTarImgWhite = (tarImg.at<uchar>(y, x) == 255);

            if (!isMetaImgWhite && !isTarImgWhite) {
                tp++; // 真陽性
            }
            else if (!isMetaImgWhite && isTarImgWhite) {
                fp++; // 偽陽性
            }
            else if (isMetaImgWhite && !isTarImgWhite) {
                fn++; // 偽陰性
