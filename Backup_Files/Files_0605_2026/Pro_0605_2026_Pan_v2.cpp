...
void multiProcess(Mat imgArr[][2]) {
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_0605_2026_v2/output/f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0605_2026_v2/output/maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_sys = nullptr;
    errno_t err3 = fopen_s(&fl_printTree_sys, "./imgs_0605_2026_v2/output/printed_tree_sys.txt", "w");
    if (err3 != 0 || fl_printTree_sys == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_read = nullptr;
    errno_t err4 = fopen_s(&fl_printTree_read, "./imgs_0605_2026_v2/output/printed_tree_read.txt", "w");
    if (err4 != 0 || fl_printTree_read == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logOptiGA = nullptr;
    errno_t err5 = fopen_s(&fl_logOptiGA, "./imgs_0605_2026_v2/output/log_opti_ga.txt", "a");
    if (err5 != 0 || fl_logOptiGA == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logPrune = nullptr;
    errno_t err6 = fopen_s(&fl_logPrune, "./imgs_0605_2026_v2/output/log_prune.txt", "a");
...
#pragma omp parallel for
    ...
    for (int idxSet = 0; idxSet < TRAIN_SIZE; idxSet++) {
        ...
        /*
        sprintf_s(imgName_pro[idxSet], "./imgs_0605_2026_v2/output/img_0%d/Gen-Final.png", idxSet + 1);
        imwrite(imgName_pro[idxSet], res);
        Mat concat;
        vector<Mat> vec = { res, imgArr[idxSet][1] };
        hconcat(vec, concat);
        sprintf_s(imgName_final[idxSet], "./imgs_0605_2026_v2/output/img_0%d/imgs_final.png", idxSet + 1);
        imwrite(imgName_final[idxSet], concat);
        */
        sprintf_s(imgName_pro[idxSet], "./imgs_0605_2026_v2/output/resImgs/crack_%05d.png", idxSet + 1);
        imwrite(imgName_pro[idxSet], res);
    }
...
int main(void) {
    ...
    /*
    if (TRAIN_SIZE == 1) {
        sprintf_s(inputPathName_ori, "./imgs_0605_2026_v2/input/oriImg_0%d.png", idSet);
        sprintf_s(inputPathName_tar, "./imgs_0605_2026_v2/input/tarImg_0%d.png", idSet);
        imgArr[0][0] = imread(inputPathName_ori, 0);
        imgArr[0][1] = imread(inputPathName_tar, 0);
    }
    else {
        for (int i = 0; i < TRAIN_SIZE; i++) {
            sprintf_s(inputPathName_ori, "./imgs_0605_2026_v2/input/oriImg_0%d.png", i + 1);
            sprintf_s(inputPathName_tar, "./imgs_0605_2026_v2/input/tarImg_0%d.png", i + 1);
