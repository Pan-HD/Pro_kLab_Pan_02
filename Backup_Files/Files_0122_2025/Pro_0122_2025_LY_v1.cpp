...
#define num_gen 400 // the nums of generation of the GA algorithm
...
typedef struct {
    ...
    int binary;
    double linear;
    char filterswitch_flag[256];
    char abusolute[256];
    char pixellabelingmethod[256];
    char erodedilate_sequence[256];
    int erodedilate_times;
    char judgeStr[8];
}deciTreeDocType;

deciTreeDocType deciTreeDocItem;
...
void import_para(int ko) {
    // Limiting the value of fsize in [7, 9, ..., 127]
    // fsize = 1 + 2 * h[ko][0].fitness;
    do {
        fsize = rand() % 121 + 7;
    } while (fsize % 2 == 0);

    // Limiting the value of binary in [160, 200]
...
void calculateMetrics(FILE* fl_deciTree, Mat metaImg[], Mat tarImg[], Mat maskImg[], int numInd, int numGen) {
    ...
    const char* src1 = "median-filter";
    const char* src2 = "average-filter";
    const char* src3 = "non-abusolute";
    const char* src4 = "abusolute";
    const char* src5 = "4pixel";
    const char* src6 = "8pixel";
    const char* src7 = "dilatefirst";
    const char* src8 = "erodefirst";
    const char* src9 = "NO";
    // const char* src10 = "YES";
    const char* src10 = "NO";

    // documents for decision tree
    // fsize, binary, linear, filterswitch_flag(1->med, 0->blur), abusolute_flag(0->non), pixellabelingmethod(0->4, 1->8)
    // erodedilate_sequence(0->dilFir), erodedilate_times, Flag
    deciTreeDocItem.fsize = fsize;
    deciTreeDocItem.binary = binary;
    deciTreeDocItem.linear = (double)linear;
    if (filterswitch_flag) {
        strcpy_s(deciTreeDocItem.filterswitch_flag, sizeof(deciTreeDocItem.filterswitch_flag), src1);
    }
    else {
        strcpy_s(deciTreeDocItem.filterswitch_flag, sizeof(deciTreeDocItem.filterswitch_flag), src2);
    }
    if (!abusolute_flag) {
        strcpy_s(deciTreeDocItem.abusolute, sizeof(deciTreeDocItem.abusolute), src3);
    }
    else {
        strcpy_s(deciTreeDocItem.abusolute, sizeof(deciTreeDocItem.abusolute), src4);
    }
    if (!pixellabelingmethod) {
        strcpy_s(deciTreeDocItem.pixellabelingmethod, sizeof(deciTreeDocItem.pixellabelingmethod), src5);
    }
    else {
        strcpy_s(deciTreeDocItem.pixellabelingmethod, sizeof(deciTreeDocItem.pixellabelingmethod), src6);
    }
    if (!erodedilate_sequence) {
        strcpy_s(deciTreeDocItem.erodedilate_sequence, sizeof(deciTreeDocItem.erodedilate_sequence), src7);
    }
    else {
        strcpy_s(deciTreeDocItem.erodedilate_sequence, sizeof(deciTreeDocItem.erodedilate_sequence), src8);
    }
    deciTreeDocItem.erodedilate_times = erodedilate_times;
    if (sum_f1 < 2.0) {
        strcpy_s(deciTreeDocItem.judgeStr, sizeof(deciTreeDocItem.judgeStr), src9);
    }
    else {
        strcpy_s(deciTreeDocItem.judgeStr, sizeof(deciTreeDocItem.judgeStr), src10);
    }
    fprintf(fl_deciTree, "%d,%d,%.2f,%s,%s,%s,%s,%d,%s\n", deciTreeDocItem.fsize, deciTreeDocItem.binary, deciTreeDocItem.linear, deciTreeDocItem.filterswitch_flag, deciTreeDocItem.abusolute, deciTreeDocItem.pixellabelingmethod, deciTreeDocItem.erodedilate_sequence, deciTreeDocItem.erodedilate_times, deciTreeDocItem.judgeStr);
}
...
void multiProcess(Mat imgArr[][3]) {
    ...
    FILE* fl_deciTree = nullptr;
    errno_t err3 = fopen_s(&fl_deciTree, "./imgs_1209_v1/output/decision_tree_30.txt", "a");
    if (err2 != 0 || fl_deciTree == nullptr) {
        perror("Cannot open the file");
        return;
    }
    ...
    for (int numGen = 1; numGen <= num_gen; numGen++) {
        ...
        for (int numInd = 0; numInd < num_ind; numInd++) {
            ...
            calculateMetrics(fl_deciTree, labelImg, tarImg, maskImg, numInd, numGen);
        }
...
int main(void) {
    ...
    deciTreeDocItem.fsize = 0;
    deciTreeDocItem.binary = 0;
    deciTreeDocItem.linear = 0.0;
    deciTreeDocItem.erodedilate_times = 0;
