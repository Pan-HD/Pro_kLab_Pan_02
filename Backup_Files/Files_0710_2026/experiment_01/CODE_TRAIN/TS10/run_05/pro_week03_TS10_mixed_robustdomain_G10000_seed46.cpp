...
static_assert(TRAIN_SIZE == 10, "This mixed-set source file expects TRAIN_SIZE=10.");
...
#ifndef GENERATIONS
#define GENERATIONS 10000 // Week-03 TS10 mixed-robustdomain formal run
#endif
...
#define WEEK03_OUTPUT_PREFIX "week03_TS10_mixed_robustdomain_GP_G10000_seed" STR(RANDOM_SEED)
#endif
...
//   /DTRAIN_SIZE=20 /DGENERATIONS=1000 /DWEEK01_ENABLE_GA=1 /DRANDOM_SEED=46
...
#if USE_ROBUST_DOMAIN_FITNESS
    // Stronger validation objective for mixed thin/thick TS10:
...
void multiProcess(Mat imgArr[][2]) {
    ...
    errno_t err = fopen_s(&fl_fValue, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_f_value.txt", "w");
    ...
    errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_maxFvalInfo_final.txt", "w");
    if (err2 != 0 || fl_maxFval == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_sys = nullptr;
    errno_t err3 = fopen_s(&fl_printTree_sys, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_printed_tree_sys.txt", "w");
    if (err3 != 0 || fl_printTree_sys == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_printTree_read = nullptr;
    errno_t err4 = fopen_s(&fl_printTree_read, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_printed_tree_read.txt", "w");
    if (err4 != 0 || fl_printTree_read == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logOptiGA = nullptr;
    errno_t err5 = fopen_s(&fl_logOptiGA, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_log_opti_ga.txt", "a");
    if (err5 != 0 || fl_logOptiGA == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_logPrune = nullptr;
    errno_t err6 = fopen_s(&fl_logPrune, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_log_prune.txt", "a");
    if (err6 != 0 || fl_logPrune == nullptr) {
        perror("Cannot open the file");
    }

    FILE* fl_debugScore = nullptr;
    errno_t err7 = fopen_s(&fl_debugScore, "./imgs_0710_2026_v1/output/train_output/" WEEK03_OUTPUT_PREFIX "_debug_rescore.txt", "w");
...
#pragma omp parallel for
    ...
    if (!genInfo.empty()) {
        double finalEliteF1[TRAIN_SIZE];
        int finalEliteIndex = genInfo.back().eliteIndex;
        for (int idxSet = 0; idxSet < TRAIN_SIZE; idxSet++) {
            finalEliteF1[idxSet] = indFValInfo[finalEliteIndex][idxSet];
        }
        printDomainFitnessCheck("[FINAL-DOMAIN-CHECK]",
            finalEliteF1,
            indFValInfo[finalEliteIndex][TRAIN_SIZE]);
    }
    ...
    for (int idxSet = 0; idxSet < TRAIN_SIZE; idxSet++) {
        ...
        sprintf_s(imgName_pro[idxSet],
            "./imgs_0710_2026_v1/output/train_output/resImgs/" WEEK03_OUTPUT_PREFIX "_crack_%05d.png",
            getTrainImageId(idxSet));
...
int main(void) {
    ...
    */
    ...
    for (int i = 0; i < TRAIN_SIZE; i++) {
        sprintf_s(inputPathName_ori, "./imgs_0710_2026_v1/input/train/positive/images/crack_%05d.png", getTrainImageId(i));
        sprintf_s(inputPathName_tar, "./imgs_0710_2026_v1/input/train/positive/masks/crack_%05d.png", getTrainImageId(i));
        imgArr[i][0] = imread(inputPathName_ori, 0);
        imgArr[i][1] = imread(inputPathName_tar, 0);
    }

    printf("[TRAIN-SET] explicit mixed IDs and domains:\n");
    for (int i = 0; i < TRAIN_SIZE; i++) {
        printf("  idx=%02d image=crack_%05d domain=%s\n",
            i, getTrainImageId(i), getTrainDomainName(i));
    }
