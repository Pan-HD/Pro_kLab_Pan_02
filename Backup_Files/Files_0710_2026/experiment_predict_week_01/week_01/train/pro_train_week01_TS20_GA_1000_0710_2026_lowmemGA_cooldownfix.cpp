// Week-01 GA overhead calibration fixed build
// Experiment: TRAIN_SIZE=20, GENERATIONS=1000, GP + GA, PRUNING disabled.
// Generated from pro_train_week01_0710_2026.cpp for estimating M_GA.
// Stable path: USE_CUDA=1, USE_FUSED_METRICS=1, ENABLE_CC_FILTER=1.

// Week-01 GP/CUDA calibration version - low-memory GA retry
// SAFE-GA patch: normalize GA candidate parameters and penalize invalid candidates instead of terminating
...
#ifndef GENERATIONS
#define GENERATIONS 1000 // Week-01 GA overhead calibration
#endif
...
#define WEEK01_RUN_LABEL "TS20_GA_G1000_SEED42_LOWMEMGA"
#endif

#ifndef WEEK01_SUMMARY_CSV
#define WEEK01_SUMMARY_CSV "./imgs_0710_2026_v1/output/train_output/week01_ga_lowmem_summary.csv"
#endif

// Low-memory GA safeguards for Week-01 GA calibration.
// OpenCV CUDA median filter can allocate large wavelet-matrix buffers during repeated GA scoring.
#ifndef WEEK01_MEDIAN_K_MAX
#define WEEK01_MEDIAN_K_MAX 9
#endif

#ifndef WEEK01_ENABLE_MEDIAN_FILTER_CACHE
#define WEEK01_ENABLE_MEDIAN_FILTER_CACHE 0
#endif

// For calibration runs, do not allow GA to re-trigger immediately after a success.
// This avoids clusters of expensive GA calls and GPU memory pressure.
#ifndef WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER
#define WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER 1
#endif
...
void initParamDesc() {
    ...
    g_paramDesc[MED_BLUR] = { 1, 3.0, static_cast<double>(WEEK01_MEDIAN_K_MAX) };
...
void initParamDesc_safeVal() {
    ...
    g_paramDesc_safeVal[MED_BLUR].push_back(static_cast<double>(WEEK01_MEDIAN_K_MAX)); // kernel size
...
Mat executeTree(const shared_ptr<TreeNode>& node, const Mat& input) {
    ...
    case MED_BLUR: {
        Mat child = executeTree(node->children[0], input);
        int k = max(3, int(node->params.size() > 0 ? int(node->params[0]) : 3));
        if ((k % 2) == 0) k |= 1;
        k = std::min(k, 31);
        Mat dst;
...
#if USE_CUDA
void clearCudaFilterCachesWeek01()
{
    try {
        getCudaStream().waitForCompletion();
        gGaussianCache.clear();
        gMedianCache.clear();
        gBoxCache.clear();
        gSobelCache.clear();
        gCannyCache.clear();
        gMorphologyCache.clear();
        cudaDeviceSynchronize();
    }
    catch (...) {
        // Best-effort cleanup only. Do not fail the training run from cleanup.
    }
}
#endif
...
cv::cuda::GpuMat executeTreeCUDA(const shared_ptr<TreeNode>& node, const cv::cuda::GpuMat& input, CudaEvalContext& ctx)
{
    ...
    try
    {
        ...
        case MED_BLUR:
        {
            ...
            if ((k % 2) == 0)
                k |= 1;
            k = std::min(k, WEEK01_MEDIAN_K_MAX);
            cv::cuda::GpuMat dst;

#if WEEK01_ENABLE_MEDIAN_FILTER_CACHE
            auto filter = getMedianFilter(child.type(), k);
#else
            auto filter = cv::cuda::createMedianFilter(child.type(), k);
#endif
...
// =====================================================
// Parameter safety utilities for GA candidates
// =====================================================
static inline double clampDoubleLocal(double v, double lo, double hi)
{
    if (v < lo) return lo;
    if (v > hi) return hi;
    return v;
...
static inline int clampOddIntLocal(double v, int lo, int hi)
{
    int k = int(v);
    if (k < lo) k = lo;
    if (k > hi) k = hi;
...
void normalizeNodeParams(shared_ptr<TreeNode>& node)
{
    if (!node) return;
    switch (node->type) {
    case GAUSSIAN_BLUR:
        if (node->params.size() >= 1) node->params[0] = double(clampOddIntLocal(node->params[0], 1, 31));
        if (node->params.size() >= 2) node->params[1] = clampDoubleLocal(node->params[1], 0.1, 31.0);
        break;
    case MED_BLUR:
        if (node->params.size() >= 1) node->params[0] = double(clampOddIntLocal(node->params[0], 3, WEEK01_MEDIAN_K_MAX));
        break;
    case BLUR:
        if (node->params.size() >= 1) node->params[0] = double(clampOddIntLocal(node->params[0], 1, 31));
        break;
    case BILATERAL_FILTER:
        if (node->params.size() >= 1) node->params[0] = double(clampOddIntLocal(node->params[0], 1, 31));
        if (node->params.size() >= 2) node->params[1] = clampDoubleLocal(node->params[1], 1.0, 150.0);
        if (node->params.size() >= 3) node->params[2] = clampDoubleLocal(node->params[2], 1.0, 150.0);
        break;
    case SOBEL_X:
    case SOBEL_Y:
        if (node->params.size() >= 1) node->params[0] = double(clampOddIntLocal(node->params[0], 1, 7));
        break;
    case CANNY:
        if (node->params.size() >= 1) node->params[0] = clampDoubleLocal(node->params[0], 1.0, 255.0);
        if (node->params.size() >= 2) node->params[1] = clampDoubleLocal(node->params[1], 1.0, 255.0);
        break;
    case THRESHOLD:
        if (node->params.size() >= 1) node->params[0] = clampDoubleLocal(node->params[0], 0.0, 255.0);
        break;
    case ERODE:
    case DILATE:
        if (node->params.size() >= 1) node->params[0] = clampDoubleLocal(node->params[0], 0.0, 5.0);
        break;
    case CC_FILTER:
        if (node->params.size() >= 1) node->params[0] = clampDoubleLocal(node->params[0], 1.0, 100.0);
        if (node->params.size() >= 2) node->params[1] = clampDoubleLocal(node->params[1], 0.0, 100.0);
        break;
    default:
        break;
    }
    for (auto& c : node->children) {
        normalizeNodeParams(c);
    }
}

static inline double safeScoreForGACandidate(const shared_ptr<TreeNode>& tree, Mat imgArr[][2])
{
    try {
        return CAL_SCORE(tree, imgArr, -1);
    }
    catch (const cv::Exception& e) {
        fprintf(stderr, "[GA-CANDIDATE-PENALTY] OpenCV exception: %s\n", e.what());
        return 0.01;
    }
    catch (const std::exception& e) {
        fprintf(stderr, "[GA-CANDIDATE-PENALTY] std exception: %s\n", e.what());
        return 0.01;
    }
    catch (...) {
        fprintf(stderr, "[GA-CANDIDATE-PENALTY] unknown exception\n");
        return 0.01;
    }
}
...
vector<double> runGrayGA_forTree(shared_ptr<TreeNode> rootInGP, Mat imgArr[][2]) {
    ...
    for (int idxInd = 0; idxInd < GA_POP; idxInd++) {
        ...
        normalizeNodeParams(rootCloned);
        pop[idxInd].fit = safeScoreForGACandidate(rootCloned, imgArr);
    }
    ...
    for (int gen = 0; gen < GA_GENERATIONS; ++gen) {
        ...
        while ((int)newpop.size() < GA_POP) {
            ...
            for (auto& pn : nodesToSet)
                for (auto& p : pn->params) p = arrDecoded[pos++];
            normalizeNodeParams(rootCloned);
            child.fit = safeScoreForGACandidate(rootCloned, imgArr);
                ...
                bias < biasThreshold)
            {
                ...
                if (lastGenFailGA < 0 || (lastGenFailGA + NUM_DELAY_GA) < numGen) {
                    ...
                    if (!bestParams.empty()) {
                        ...
                        normalizeNodeParams(eliteTreeClone);

                        double scoreAfterGA =
                            safeScoreForGACandidate(eliteTreeClone, imgArr);
...
#if WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER
                            lastGenFailGA = numGen;
                            printf("[GA-COOLDOWN-AFTER-SUCCESS] gen=%d cooldownUntilGen=%d\n",
                                numGen + 1,
                                numGen + 1 + NUM_DELAY_GA);
#endif
...
#if !WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER
                            // Original behavior: after a GA success, clear the failure cooldown.
                            // When WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER=1, keep lastGenFailGA as
                            // the last GA trigger generation so that success also enters cooldown.
                            if (lastGenFailGA >= 0) {
                                lastGenFailGA = -1;
                            }
#endif
...
#if WEEK01_GA_COOLDOWN_AFTER_ANY_TRIGGER
                        lastGenFailGA = numGen;
                        printf("[GA-COOLDOWN-AFTER-EMPTY] gen=%d cooldownUntilGen=%d\n",
                            numGen + 1,
                            numGen + 1 + NUM_DELAY_GA);
#endif
                    }
